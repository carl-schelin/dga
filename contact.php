...
#  Function: Provide contact information for the convention
#  Description: This lists the contact information for the convention as well as for individual coordinators
#    if the site is not something a gamer wishes to use.
...
# perhaps 'signup' logs the user in as a link.
  if (isset($_SESSION['username'])) {
    ...
    logaccess($_SESSION['username'], $package, "View contact info");
...
<?php
        $q_string = "select con_text from contacts where con_status > 0 and con_conid = " . $a_convention['con_id'];
        $q_contacts = mysql_query($q_string) or die($q_string . ": " . mysql_error());
        while ($a_contacts = mysql_fetch_array($q_contacts)) {
          $a_contacts['con_text'] = str_replace("<p>", "<p class=\"contacts\">", $a_contacts['con_text']);
          print $a_contacts['con_text'];
        }

        $q_string = "select cat_base,cat_title,cat_email,cat_summary from categories where cat_status > 0 and cat_conid = " . $a_convention['con_id'] . " order by cat_base";
        $q_categories = mysql_query($q_string) or die($q_string . ": " . mysql_error());
        while ($a_categories = mysql_fetch_array($q_categories)) {
          print "<p class=\"contacts\"><strong>" . $a_categories['cat_base'] . " - " . $a_categories['cat_title'] . " - (" . $a_categories['cat_email'] . ")</strong></p>\n";
          $a_categories['cat_summary'] = str_replace("<p>", "<p class=\"contacts\">", $a_categories['cat_summary']);
          print "<p class=\"contacts\">" . $a_categories['cat_summary'] . "</p>\n";
        }
?>
      </div>
    </div>  
