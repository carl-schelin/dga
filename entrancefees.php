...
session_start();
#
#  Package: entrancefees.php
#  Version: 1.0.0
#  Author: Carl Schelin, Copyright 2013. All rights reserved.
#  Function: Show the fees for access to the convention
#  Description: This lists the fees for access
...
# perhaps 'signup' logs the user in as a link.
  if (isset($_SESSION['username'])) {
    ...
    logaccess($_SESSION['username'], $package, "View fees");
...
<?php
        $q_string = "select fee_text from fees where fee_status > 0 and fee_conid = " . $a_convention['con_id'];
        $q_fees = mysql_query($q_string) or die($q_string . ": " . mysql_error());
        while ($a_fees = mysql_fetch_array($q_fees)) {
          $a_fees['fee_text'] = str_replace("<p>", "<p class=\"fees\">", $a_fees['fee_text']);
          print $a_fees['fee_text'];
        }
?>
      </div>
    </div>  
