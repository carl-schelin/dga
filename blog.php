...
#  Function: Provide review of blog postings
#  Description: 
...
# perhaps 'signup' logs the user in as a link.
  if (isset($_SESSION['username'])) {
    ...
    logaccess($_SESSION['username'], $package, "View blog");
...
<?php
  $q_string = "select news_title,news_text from news where news_status > 0 order by news_start desc";
  $q_news = mysql_query($q_string) or die($q_string . ": " . mysql_error());
  while ($a_news = mysql_fetch_array($q_news)) {
    print "<h1>" . $a_news['news_title'] . "</h1>\n";
